...
def _retry_delay(exc: Exception, attempt: int) -> float:
    """Use the provider's retry hint, falling back to jittered backoff."""
    ...
    for header_name in ("retry-after-ms", "retry-after"):
        ...
        if raw_value is None:
            continue
        try:
            delay = float(raw_value) / (1000 if header_name.endswith("-ms") else 1)
            return max(0.0, delay)
        except (TypeError, ValueError):
            continue

    backoff = RETRY_BASE_DELAY * (2 ** attempt)
    return backoff + random.uniform(0, RETRY_BASE_DELAY)
...
class Agent:
    ...
    def _as_message(model_message) -> Message:
        ...
        # Gemini uses this metadata to validate tool calls across turns.
        extra_content = getattr(model_message, "extra_content", None)
        if extra_content:
